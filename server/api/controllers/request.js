...
export const getAllRequests = async (req, res) => {
  try {
    const {
      ...
      toDate,          // עד תאריך
      city,            // עיר מגורים
      minSiblings,     // מינימום אחים מתחת לגיל 18
      maxSiblings,     // מקסימום אחים מתחת לגיל 18
      minTuition,      // שכר לימוד מינימום
      maxTuition,      // שכר לימוד מקסימום
      sortBy,          // 'date' | 'siblings' | 'tuition'
      sortOrder,       // 'asc' | 'desc'
    } = req.query;

   const filter = {
   status: 'submitted'  // מציג רק בקשות בהמתנה
};

    // חיפוש לפי מ.ז
    if (id) {
      filter['personalDetails.id'] = { $regex: id, $options: 'i' };
    }

    // טווח תאריכים
    if (fromDate || toDate) {
      filter['submissionDate'] = {};
      if (fromDate) filter['submissionDate'].$gte = new Date(fromDate);
      if (toDate)   filter['submissionDate'].$lte = new Date(toDate);
    }
    ...
    // כמות אחים מתחת לגיל 18
    if (minSiblings || maxSiblings) {
      filter['familyDetails.siblingsBelowAge18'] = {};
      if (minSiblings) filter['familyDetails.siblingsBelowAge18'].$gte = Number(minSiblings);
      if (maxSiblings) filter['familyDetails.siblingsBelowAge18'].$lte = Number(maxSiblings);
    }

    // שכר לימוד שנתי
    if (minTuition || maxTuition) {
      filter['courseDetails.annualTuition'] = {};
      if (minTuition) filter['courseDetails.annualTuition'].$gte = Number(minTuition);
      if (maxTuition) filter['courseDetails.annualTuition'].$lte = Number(maxTuition);
    }

    // מיון
    const sortField = {
      date:     'submissionDate',
      siblings: 'familyDetails.siblingsBelowAge18',
      tuition:  'courseDetails.annualTuition',
    }[sortBy] || 'submissionDate';

    const sort = { [sortField]: sortOrder === 'asc' ? 1 : -1 };

    const requests = await Request.find(filter)
      .select('personalDetails.id personalDetails.firstName personalDetails.lastName personalDetails.city courseDetails.major courseDetails.annualTuition familyDetails.siblingsBelowAge18 status submissionDate')
      .sort(sort);
